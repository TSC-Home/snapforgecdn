...
export const handleError: HandleServerError = async ({
  ...
  console.error(
    `[${errorId}] Origin header: ${event.request.headers.get("origin")}`,
  );
  console.error(
    `[${errorId}] Host header: ${event.request.headers.get("host")}`,
  );
  console.error(
    `[${errorId}] X-Forwarded-Host: ${event.request.headers.get("x-forwarded-host")}`,
  );
  console.error(
    `[${errorId}] X-Forwarded-Proto: ${event.request.headers.get("x-forwarded-proto")}`,
  );
  ...
  if (
    ...
    console.error(
      `[${errorId}] CSRF CHECK FAILED - This is likely a reverse proxy configuration issue`,
    );
    console.error(
      `[${errorId}] Make sure ORIGIN env var matches the public URL`,
    );
