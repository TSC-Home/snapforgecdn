...
export async function inviteToGallery(
  ...
  const inviterPermissions = await getUserGalleryPermissions(
    invitedBy,
    galleryId,
  );
...
export async function getInvitationByToken(
  token: string,
): Promise<InvitationWithGallery | null> {
  const invitation = await db
    .select()
    .from(schema.galleryInvitations)
    .where(eq(schema.galleryInvitations.token, token))
...
export async function getGalleryCollaborators(
  galleryId: string,
  ...
  const collaborators = await db
    ...
    .innerJoin(
      schema.users,
      eq(schema.galleryCollaborators.userId, schema.users.id),
    )
...
export async function cancelInvitation(
  ...
  // Check if requester has permission
  const permissions = await getUserGalleryPermissions(
    requesterId,
    invitation.galleryId,
  );
  if (!permissions?.canManageCollaborators) {
    return false;
  }
...
export async function getUserAccessibleGalleries(
  userId: string,
): Promise<{ gallery: Gallery; role: CollaboratorRole | "owner" }[]> {
  // Get owned galleries
  ...
  const collaborations = await db
    ...
    .innerJoin(
      schema.galleries,
      eq(schema.galleryCollaborators.galleryId, schema.galleries.id),
    )
