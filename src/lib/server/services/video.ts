...
export async function processVideo(
	...
	return new Promise((resolve, reject) => {
		...
		if (audioCodec !== 'none' && audioCodec !== 'copy') {
			const audioBitrate = options.audioBitrate || config.videos.defaultAudioBitrate;
...
function buildScaleFilter(maxWidth?: number | null, maxHeight?: number | null): string {
...
export async function uploadVideo(
	...
	const tempPath = join(tempDir, `${videoId}_original.${ext}`);
	...
	try {
		// Get video metadata
		const fullTempPath = join(config.storage.localPath, `temp/${videoId}_original.${ext}`);
		...
		if (metadata.duration > maxDuration) {
			...
			return { success: false, error: `Video too long. Maximum duration is ${Math.round(maxDuration / 60)} minutes` };
		}
		...
		const needsProcessing = gallery.videoOutputFormat || gallery.videoCodec ||
			gallery.videoMaxWidth || gallery.videoMaxHeight;
		...
		if (needsProcessing) {
			...
			await processVideo(fullStoragePath, fullProcessedPath, {
				outputFormat: gallery.videoOutputFormat === 'original' ? null : gallery.videoOutputFormat,
...
// Delete video
export async function deleteVideo(videoId: string, galleryId: string): Promise<{ success: boolean; error?: string }> {
	const video = await db
		.select()
		.from(schema.videos)
		.where(and(eq(schema.videos.id, videoId), eq(schema.videos.galleryId, galleryId)))
		.get();
...
export async function getVideoBuffer(videoId: string, useProcessed: boolean = true): Promise<{
	buffer: Buffer;
	mimeType: string;
	size: number;
} | null> {
	...
	const path = useProcessed && video.processedPath ? video.processedPath : video.storagePath;

	try {
		const buffer = await storage.read(path);
		const mimeType = video.processedPath && useProcessed
			? (path.endsWith('.webm') ? 'video/webm' : 'video/mp4')
			: video.mimeType;
