<script lang="ts">
	...
	const canCreate = $derived(
		...
		!availableTags.some(t => t.name.toLowerCase() === newTagName.trim().toLowerCase())
	);
	...
	function handleKeydown(e: KeyboardEvent) {
		if (e.key === 'Enter') {
			...
			showDropdown = false;
			inputElement?.blur();
		} else if (e.key === 'Backspace' && !newTagName && selectedTagIds.length > 0) {
		...
		<div class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-48 overflow-y-auto">
			{#if canCreate}
				...
				>
					...
					<span>Create "<span class="font-medium">{newTagName.trim()}</span>"</span>
				</button>
			{/if}
