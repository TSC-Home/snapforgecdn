		...
		try {
			...
			if (res.ok) {
				...
				if (results.length > 0) {
					...
					latitude = latNum;
					longitude = lngNum;
					locationName = display_name?.split(',').slice(0, 3).join(', ') || null;
...
<div class="space-y-3">
	{#if !readonly}
		<div class="flex gap-2">
			...
			<button
				...
				disabled={searching}
				class="px-3 h-9 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-md hover:bg-gray-50 disabled:opacity-50"
			>
	...
	{#if latitude !== null && longitude !== null}
		<div class="flex items-start justify-between gap-2 p-3 bg-gray-50 rounded-lg">
			<div class="min-w-0 flex-1">
				{#if locationName}
					<p class="text-sm font-medium text-gray-900 truncate">{locationName}</p>
				{/if}
	...
	{:else}
		<p class="text-xs text-gray-500 text-center">Click on the map to set location</p>
	{/if}
</div>
