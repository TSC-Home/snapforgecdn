...
<script lang="ts">
    ...
    function isSessionComplete(session: UploadSession): boolean {
        return (
            session.successCount + session.errorCount === session.items.length
        );
    }
        ...
        >
            ...
            >
                <div class="flex items-center gap-2 min-w-0">
                    ...
                    {:else if isComplete}
                        <svg
                            class="w-5 h-5 shrink-0 {session.errorCount > 0
                                ? 'text-amber-500'
                                : 'text-emerald-500'}"
                            fill="none"
                            stroke="currentColor"
                ...
                <div class="flex items-center gap-1">
                    <button
                        ...
                        aria-label={minimized[session.id]
                            ? "Expand"
                            : "Minimize"}
                    >
                        <svg
                ...
                <div class="px-4 pt-3">
                    ...
                    >
                        <span class="text-gray-600 font-medium">
                            {session.successCount + session.errorCount} of {session
                                .items.length}
                        </span>
                        ...
                        {#if session.errorCount > 0}
                            <div class="mt-2 max-h-24 overflow-y-auto">
                                {#each session.items.filter((item) => item.status === "error") as failedItem}
                                    <div
                                        class="text-xs py-1 border-t border-gray-50"
                                    >
                                        <p class="text-gray-700 truncate">
                                            {failedItem.filename}
                                        </p>
                                        <p class="text-red-500 truncate">
                                            {failedItem.error ||
                                                "Upload failed"}
                                        </p>
                                    </div>
