<script lang="ts">
	import { Button, Input, Select, Card, Modal, Toast, toast, Table, Th, Td, Pagination, Dropdown, DropdownItem } from '$lib/components/ui';
...
<AppShell>
	...
	<div class="space-y-8 max-w-4xl">
		...
		<Modal bind:open={modalOpen} title="Example Modal" size="md">
			<p class="text-gray-600">This is the modal content. Press ESC or click outside to close.</p>
			{#snippet footer()}
				<Button variant="ghost" onclick={() => modalOpen = false}>Cancel</Button>
				<Button onclick={() => { modalOpen = false; toast('Saved!', 'success'); }}>Save</Button>
			{/snippet}
		</Modal>
		...
		<Card>
			...
			<div class="flex flex-wrap gap-2">
				<Button variant="secondary" onclick={() => toast('Info message', 'info')}>Info Toast</Button>
				<Button variant="secondary" onclick={() => toast('Success message', 'success')}>Success Toast</Button>
				<Button variant="secondary" onclick={() => toast('Error message', 'error')}>Error Toast</Button>
			</div>
		</Card>
		...
		<Card>
			...
			<div class="flex gap-4">
				<Dropdown align="left">
					...
					{/snippet}
					<DropdownItem onclick={() => toast('Edit clicked', 'info')}>Edit</DropdownItem>
					<DropdownItem onclick={() => toast('Duplicate clicked', 'info')}>Duplicate</DropdownItem>
					<DropdownItem danger onclick={() => toast('Delete clicked', 'error')}>Delete</DropdownItem>
				</Dropdown>
