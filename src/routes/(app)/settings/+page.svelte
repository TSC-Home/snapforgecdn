...
<div class="max-w-3xl space-y-6">
	...
	<div>
		...
		<p class="text-sm text-gray-500">Manage your account and system configuration</p>
	</div>
		...
		<nav class="flex gap-1">
			...
			{#if data.isAdmin}
				<button
					...
					class="relative px-4 py-2.5 text-sm font-medium rounded-t-md transition-all duration-200 flex items-center gap-2
						{activeTab === 'admin'
							? 'text-gray-900 bg-white border border-b-white border-gray-200 -mb-px'
							: 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'}"
				>
					Admin
					<span class="px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide rounded bg-amber-100 text-amber-700">
						System
					</span>
				</button>
			{/if}
			...
			<Card>
				...
				<div class="grid grid-cols-2 sm:grid-cols-3 gap-6">
					<div>
						...
						<p class="mt-1 text-sm font-medium text-gray-900 capitalize">{data.user?.role}</p>
					</div>
					<div>
						<span class="text-xs font-medium text-gray-400 uppercase tracking-wide">Gallery Limit</span>
						<p class="mt-1 text-sm font-medium text-gray-900">{data.user?.maxGalleries}</p>
					</div>
					<div>
						<span class="text-xs font-medium text-gray-400 uppercase tracking-wide">Member Since</span>
						<p class="mt-1 text-sm font-medium text-gray-900">{data.user?.createdAt ? formatDate(data.user.createdAt) : '—'}</p>
					</div>
				</div>
			</Card>
					...
					<div class="pt-4 border-t border-gray-100">
						<label class="flex items-start gap-3 cursor-pointer group">
							...
							<div class="flex-1">
								...
								<p class="text-sm text-gray-500">Enable self-service account creation for new users.</p>
							</div>
						</label>
					</div>
			...
			<Card>
				...
				>
					...
					{#if storageType === 's3'}
						...
						<div class="flex items-start gap-2 p-3 bg-amber-50 border border-amber-100 rounded-lg">
							<svg class="w-4 h-4 text-amber-500 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
							</svg>
							<p class="text-sm text-amber-700">
								Existing images must be migrated manually after switching storage backends.
							</p>
						</div>
					{/if}
					...
					<!-- Hidden fields -->
					{#if storageType === 'local'}
						<input type="hidden" name="s3Bucket" value={data.settings.storage.s3Bucket} />
						<input type="hidden" name="s3Region" value={data.settings.storage.s3Region} />
						<input type="hidden" name="s3AccessKey" value={data.settings.storage.s3AccessKey} />
						<input type="hidden" name="s3SecretKey" value={data.settings.storage.s3SecretKey} />
						<input type="hidden" name="s3Endpoint" value={data.settings.storage.s3Endpoint} />
					{:else}
						<input type="hidden" name="localPath" value={data.settings.storage.localPath} />
					{/if}
