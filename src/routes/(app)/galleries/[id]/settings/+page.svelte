<script lang="ts">
    ...
    // General settings
    let galleryName = $derived(data.gallery.name);

    // Processing settings
    let thumbSize = $derived(data.gallery.thumbSize?.toString() ?? "");
    let thumbQuality = $derived(data.gallery.thumbQuality?.toString() ?? "");
    let outputFormat = $derived(data.gallery.outputFormat ?? "");
    let resizeMethod = $derived(data.gallery.resizeMethod ?? "");
    let jpegQuality = $derived(data.gallery.jpegQuality?.toString() ?? "");
    let webpQuality = $derived(data.gallery.webpQuality?.toString() ?? "");
    let avifQuality = $derived(data.gallery.avifQuality?.toString() ?? "");
    let pngCompressionLevel = $derived(
        data.gallery.pngCompressionLevel?.toString() ?? "",
    );
    let effort = $derived(data.gallery.effort?.toString() ?? "");
    let chromaSubsampling = $derived(data.gallery.chromaSubsampling ?? "");
    let stripMetadata = $derived(data.gallery.stripMetadata ?? false);
    let autoOrient = $derived(data.gallery.autoOrient ?? true);

    // Video processing settings
    let videoEnabled = $derived(data.gallery.videoEnabled ?? true);
    let videoMaxSize = $derived(data.gallery.videoMaxSize?.toString() ?? "");
    let videoMaxDuration = $derived(
        data.gallery.videoMaxDuration?.toString() ?? "",
    );
    let videoOutputFormat = $derived(data.gallery.videoOutputFormat ?? "");
    let videoCodec = $derived(data.gallery.videoCodec ?? "");
    let videoQuality = $derived(data.gallery.videoQuality?.toString() ?? "");
    let videoMaxWidth = $derived(data.gallery.videoMaxWidth?.toString() ?? "");
    let videoMaxHeight = $derived(
        data.gallery.videoMaxHeight?.toString() ?? "",
    );
    let videoAudioCodec = $derived(data.gallery.videoAudioCodec ?? "");
    let videoAudioBitrate = $derived(
        data.gallery.videoAudioBitrate?.toString() ?? "",
    );
    let videoGenerateThumbnail = $derived(
        data.gallery.videoGenerateThumbnail ?? true,
    );
    let videoThumbnailTime = $derived(
        data.gallery.videoThumbnailTime?.toString() ?? "",
    );
...
<div class="space-y-6">
    ...
    <div>
        <a
            href="/galleries/{data.gallery.id}"
            class="inline-flex items-center gap-1 text-sm text-gray-400 hover:text-gray-600 transition-colors mb-2"
        >
            <svg
                class="w-4 h-4"
                fill="none"
        ...
        <p class="text-sm text-gray-500 mt-1">
            Configure gallery options and image processing
        </p>
    </div>
            ...
            <Card>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-6">
                    ...
                    <div>
                        ...
                        >
                        <p class="mt-1 text-sm font-medium text-gray-900">
                            {formatDate(data.gallery.createdAt)}
                        </p>
                    </div>
                    <div>
                        <span
            ...
            <Card>
                ...
                >
                    {#if form?.error && form?.action === "rename"}
                        <div
                            class="p-3 bg-red-50 border border-red-100 text-red-600 text-sm rounded-lg"
                        >
                            {form.error}
                        </div>
                    {/if}
            ...
            <Card>
                ...
                >
                    ...
                    <p class="text-sm text-gray-600">
                        Configure how images are processed when uploaded. Leave
                        fields empty to use system defaults.
                    </p>
                    ...
                    <div class="space-y-4 pb-5 border-b border-gray-100">
                        ...
                        <Select
                            ...
                            options={[
                                ...
                                {
                                    value: "jpeg",
                                    label: "JPEG - Best compatibility",
                                },
                                {
                                    value: "webp",
                                    label: "WebP - Modern format, smaller files",
                                },
                                {
                                    value: "avif",
                                    label: "AVIF - Best compression (slower)",
                                },
                        ...
                        <div class="grid sm:grid-cols-2 gap-4">
                            <Select
                                ...
                                options={[
                                    ...
                                    {
                                        value: "lanczos3",
                                        label: "Lanczos3 - Best quality",
                                    },
                                    {
                                        value: "lanczos2",
                                        label: "Lanczos2 - Faster",
                                    },
                                    {
                                        value: "mitchell",
                                        label: "Mitchell - Balanced",
                                    },
                                    {
                                        value: "nearest",
                                        label: "Nearest - Fastest",
                                    },
                                ]}
                            />
                            <Select
                                ...
                                options={[
                                    ...
                                    {
                                        value: "420",
                                        label: "4:2:0 - Smaller files",
                                    },
                                    { value: "422", label: "4:2:2 - Balanced" },
                                    {
                                        value: "444",
                                        label: "4:4:4 - Best quality",
                                    },
                                ]}
                            />
                            <Input
                    ...
                    <div class="pt-4 border-t border-gray-100">
                        <Button type="submit" {loading}
                            >Save Processing Settings</Button
                        >
                    </div>
                </form>
            </Card>
                ...
                >
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xs text-gray-500">
                                Allow uploading video files to this gallery
                            </p>
                        </div>
                        <label
                        ...
                        >
                            ...
                            />
                            <div
                                class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-gray-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-gray-900"
                            ></div>
                        </label>
                    </div>
                    ...
                    {#if videoEnabled}
                            ...
                            <div class="grid sm:grid-cols-2 gap-4">
                                <Select
                                    ...
                                    options={[
                                        ...
                                        {
                                            value: "mp4",
                                            label: "MP4 - Best compatibility",
                                        },
                                        {
                                            value: "webm",
                                            label: "WebM - Modern browsers",
                                        },
                                    ]}
                                />
                                <Select
                                    ...
                                    options={[
                                        ...
                                        {
                                            value: "h264",
                                            label: "H.264 - Best compatibility",
                                        },
                                        {
                                            value: "h265",
                                            label: "H.265/HEVC - Better compression",
                                        },
                                        {
                                            value: "vp9",
                                            label: "VP9 - Open format",
                                        },
                                        {
                                            value: "av1",
                                            label: "AV1 - Best compression (slow)",
                                        },
                                    ]}
                                />
                            </div>
                            ...
                            <div class="grid sm:grid-cols-2 gap-4">
                                <Select
                                    ...
                                    options={[
                                        ...
                                        {
                                            value: "aac",
                                            label: "AAC - Best compatibility",
                                        },
                                        {
                                            value: "opus",
                                            label: "Opus - Better quality",
                                        },
                                        {
                                            value: "copy",
                                            label: "Copy - Keep original",
                                        },
                                        {
                                            value: "none",
                                            label: "None - Remove audio",
                                        },
                                    ]}
                                />
                                <Input
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex items-stretch gap-4">
                                <label
                                    class="flex items-start gap-3 w-full cursor-pointer group flex-1"
                                >
                                    <input
                                        type="checkbox"
                                    ...
                                    <div>
                                        <span
                                            class="text-sm font-medium text-gray-700 group-hover:text-gray-900"
                                            >Generate thumbnail</span
                                        >
                                        <p class="text-xs text-gray-500">
                                            Extract a frame as preview image
                                        </p>
                                    </div>
                                </label>
    ...
    {#if activeTab === "api"}
        <div class="space-y-6 animate-in">
            <Card>
                ...
                <div class="space-y-4">
                    <p class="text-sm text-gray-600">
                        Use these credentials to authenticate API requests. Keep
                        the token secret!
                    </p>
                    ...
                    <div>
                        ...
                        <div class="mt-2 flex gap-2">
                            <code
                                class="flex-1 px-3 py-2 bg-gray-50 border border-gray-200 text-sm font-mono break-all rounded-lg"
                            >
                                {data.gallery.id}
                            </code>
                            <Button
                    ...
                    <div>
                        ...
                        >
                        <code
                            class="mt-2 block px-3 py-2 bg-gray-50 border border-gray-200 text-sm font-mono rounded-lg"
                        >
                ...
                <div class="space-y-6">
                    <div class="p-4 bg-red-50 border border-red-100 rounded-lg">
                        ...
                        <p class="text-sm text-red-700 mb-4">
                            This will permanently delete this gallery and all {data
                                .stats.imageCount} images. This action cannot be undone.
                        </p>
                        <form
                            method="POST"
                            action="?/delete"
                            use:enhance={({ cancel }) => {
                                const name = prompt(
                                    `Type "${data.gallery.name}" to confirm deletion:`,
                                );
