<script lang="ts">
  ...
  const navItems = $derived([
    ...
    },
    {
      label: "Galleries",
      href: "/galleries",
      active: $page.url.pathname.startsWith("/galleries"),
    },
    {
      label: "Settings",
      href: "/settings",
    ...
    <Header title={pageTitle()}>
      {#snippet actions()}
        <Dropdown align="right">
          {#snippet trigger()}
            <button
              class="flex items-center gap-2 px-3 py-2 text-sm text-gray-600 hover:bg-gray-100  transition-colors cursor-pointer rounded-md"
            >
